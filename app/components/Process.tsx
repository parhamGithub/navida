...
export default function Process() {
  return (
    <section id="process" className="py-[140px] px-[6%] bg-charcoal border-t border-line border-b border-line">
      <motion.div
      ...
      <div className="max-w-[760px] mx-auto">
        {steps.map((step, i) => (
          <motion.div
            ...
            className="flex gap-7 py-[30px] border-b border-line items-start last:border-none"
          >
            <div className="font-serif text-[34px] text-gold-dim font-semibold min-w-[52px]">{step.num}</div>
            <div>
              <h4 className="text-[17px] text-ivory font-bold mb-2">{step.title}</h4>
              <p className="text-[13.8px] text-muted leading-[1.9] max-w-[520px]">{step.desc}</p>
            </div>
          </motion.div>
        ))}
      </div>
